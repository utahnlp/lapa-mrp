...
class InternalTreebankNode(TreebankNode):
    ...
    def toTOP_linearize(self):
        return "[{} {} ]".format(
            self.label, " ".join(child.toTOP_linearize() for child in self.children))
...
class LeafTreebankNode(TreebankNode):
    ...
    def toTOP_linearize(self):
        if self.word in PTB_TOKEN_UNESCAPE:
            return "{}".format(PTB_TOKEN_UNESCAPE[self.word])
        else:
