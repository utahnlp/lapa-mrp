...
from utility.amr_utils.AMRGraph import *
from utility.constants import *
from utility.amr_utils.PropbankReader import PropbankReader
...
def unmixe(mixed,threshold = 50):
    ...
    high_frequency = dict()
    low_frequency = dict()
    # use text_num for all constants category
    low_text_num = dict()
    high_text_num = dict()
    for i in mixed:
        cat = i.cat
...
class AMRRules:
    ...
    def save(self,filepath="dicts/amr_rule_f"):
    ...
    lock = threading.Lock()
    def load(self,filepath="dicts/amr_rule_f"):
        pickle_helper= Pickle_Helper(filepath)
    ...
    def read_veb(self):
        """
        read amr verbliazation file, to decide whether doing verbalization on some words.
        add lemma -> verbalized_lemma into dict
        """
        RE_FRAME_NUM = re.compile(r'-\d\d$')
    ...
    def __init__(self):
        # lemma_freq_cat os [cat][old_lemma][amr_con]=freq
        self.lemma_freq_cat = {}
        # lemmatize_cheat => [cat, lemma] -> best_amr_lemma
        self.lemmatize_cheat = {}
        # lemma_back is [amr_lemma][old_lemma]=freq
        self.lemma_back = {}
        # read propbank frames and adding lemma2frame mapping
        self.read_frame()
        self.read_veb()
        # frame_lemmas is a dict, lemma -> AMRUniversal(lemma, RULE_Frame, sense)
        self.frame_lemmas = PropbankReader().frame_lemmas
        # when initial, build_lemma_cheat will only consider the propbank frame and verbalization  frames, cat is Rule_Frame
        self.build_lemma_cheat()
        self.set_rules()
    ...
    def standard_rule(self,lemma,cat,con_lemma=None,sense=NULL_WORD):
        """
        A unified method for taking lemma, cat, con_lemma, sense, to generate a AMRUniversal Node.
        which can be used for prediction for concepts, and then expanded to nodes for relations. 
        """
         ...
         #       elif lemma == "cooperation" and cat == Rule_Frame:
          #          print ("after cooperation",AMRUniversal(lemma,cat,sense))
            ...
            if cat not in self.lemma_freq_cat or lemma not in self.lemma_freq_cat[cat]:
                return AMRUniversal(lemma,cat,sense)
            # Why when training, use lemma_freq_cat?
            ...
            for word in lemma_freqs:
                ...
                for arm_le in lemma_freqs[word]:
                    ...
                    if score >max_score:
                        max_score = score
                        max_lemma = arm_le
                # here, it only keep the best amr_lemma given a key [word, cat]
   ...
   # fragments_to_break = set(["up","down","make"])
    ...
    def get_matched_concepts(self,snt_token,amr_or_node_value,lemma,pos,with_target = False,jamr=False,full=1):
        """
        Use simple distance match rules to find the alignments
        full : means merge the jamr alignment if existed.
        jamr: use jamr or not
        with_target: seems relted with le and sense, not sure what it is about.
        return [[n,c,a]]
        """
        results = []
        ...
        # n is node, c is it value AMEUniversal, a is align
        for n,c,a in node_value:
            if full == 1:
                # use distant match rule to find the possible alignments
                align = self.match_concept(snt_token,c,lemma,pos,with_target)
                # if we have jamr alignments, count jamr in.
                ...
                # not full, means either jamr alignment or matching-based alignments
                if jamr and a is not None:
                    align = [(a,lemma[a],pos[a])]
                else:
                    align = self.match_concept(snt_token,c,lemma,pos,with_target)
    ...
    def match_concept(self,snt_token,concept,lemma,pos,with_target = False,candidate = None):
        ...
        for i in candidate:
            if with_target and  disMatch(lemma[i],le) <1: # and pos[i] not in ["IN"]:
                ...
                continue
            if with_target:
                # with the corresponding label for the word
