...
import utility.dm_utils.DMGraph

class DMNaiveScores(Scorer):
    def __init(self):
        super(DMNaiveScores, self).__init__()
        self.show_error_names= ['Full Concept','Lemma Only','Cat Only','High Freq Only', 'Copy Only','Root','REL Triple given concept']

    @staticmethod
    def nonsense_concept(uni):
        return uni.le.lower(),uni.pos,uni.cat,uni.get_anchors_str()

    @staticmethod
    def nonanchors_concept(uni):
        return uni.le.lower(),uni.pos,uni.cat,uni.sense

    @staticmethod
    def nonlemma_concept(uni):
        return uni.pos,uni.cat,uni.sense, uni.get_anchors_str()

    @staticmethod
    def noncat_concept(uni):
        return uni.le.lower(), uni.pos,uni.sense, uni.get_anchors_str()
    ...
    def remove_sense(uni):
        return (uni.pos,uni.le.lower(),uni.get_anchors_str())

    #naive set overlapping for different kinds of relations
    def rel_scores_initial(self):
        nonsense_concept = DMNaiveScores.nonsense_concept
        anchoronly_concept = DMNaiveScores.anchoronly_concept
    ...
    def concept_score_initial(self, dicts):
        nonsense_concept = DMNaiveScores.nonsense_concept
        nonanchors_concept = DMNaiveScores.nonanchors_concept
        nonlemma_concept = DMNaiveScores.nonlemma_concept
        ...
        cat_score =  ScoreHelper("Cat Only",filter=lambda uni: (uni.cat) if legal_concept(uni) else None)
        high_score =  ScoreHelper("High Freq Only",filter=lambda uni: (uni.le.lower(),uni.cat) if  uni.cat in dicts["dm_high_dict"] and legal_concept(uni)  else None)
        copy_score =  ScoreHelper("Copy Only",filter=lambda uni: (uni.le.lower(),uni.cat) if  uni.cat not in dicts["dm_high_dict"] and legal_concept(uni) else None)
        # add abstract frame score
        return  [Non_Sense,Non_Anchors,Non_Lemma,concept_score,pos_score,sense_score,anchors_score,lemma_score, cat_score, high_score, copy_score]
