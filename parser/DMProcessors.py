...
DMParser for producing dm graph from raw text
DMDecoder for decoding deep learning model output into actual DM nodes and graph
DMInputPreprocessor for extract features based on stanford corenlp
@author: Chunchuan Lyu (chunchuan.lv@gmail.com)
@since: 2018-05-30

# This DMProcessors are used as a luanch for preprocessing the input raw sentence and parse it
...
from src import *
from utility.dm_utils.DMGraph import DMGraph
...
logger = logging.getLogger("mrp.DMProcessors")

class DMInputPreprocessor(object):
    """
    A feature extractor for dm input
    """
    def __init__(self, opt, url = core_nlp_url, joints_file= dm_mwe_file):
        self.nlp = StanfordCoreNLP(url)
        self.opt = opt
        # TODO: generate a joints_map by all the concepts
    ...
    def featureExtract(self,src_text,whiteSpace=True):
        """
        Using stanford nlp url to extract features from dm input text
...
class DMParser(object):
    ...
    def __init__(self, opt,dicts):
        self.decoder = DMDecoder(opt,dicts,"dm")
        self.frame = "dm"
        self.model, _, _ = load_old_model(dicts,opt,True)[0]
        self.opt = opt
        self.feature_extractor = DMInputPreprocessor(opt,core_nlp_url)
        self.dicts = dicts
        self.decoder.eval()
        self.model.eval()
    ...
    def feature_to_torch(self,all_data):
        ...
        if self.opt.bert_model:
            data_iterator = BertDataIterator([],self.opt,self.dicts["dm_rel_dict"],all_data=all_data)
            order,idsBatch,srcBatch, src_charBatch, sourceBatch,srcBertBatch = data_iterator[0]
        else:
            data_iterator = DataIterator([],self.opt,self.dicts["dm_rel_dict"],all_data=all_data)
    ...
    def parse_batch_preprocessed_data(self, preprocessed_input_examplesi, set_wiki, normalize_mod):
        ...
        for i,data in enumerate(zip(idsBatch, sourceBatch,concepts_pred_seq,concept_batches,rel_triples,graphs)):
            ...
            mrp_graph, catd_graph = self.decoder.graph_to_mrpGraph(example_id, graph, normalizeMod = opt.normalize_mod, flavor=0, framework="dm", sentence=" ".join(source[0]))
...
class DMDecoder(object):
    def __init__(self, opt,dicts):
        """
        init the model for DM Decoder in pytorch
        Especially, it relys on the dicts/vocabularies for tokens and labels.
        """
        self.opt = opt
        self.dicts = dicts
        self.frame = "dm"
        self.n_high = len(dicts["dm_high_dict"])
        #self.n_le_high = len(dicts["dm_high_le_dict"])
        self.rl = DMRules()
        self.rl.load(opt.build_folder+"dicts/dm_rule_f")
        self.fragment_to_node_converter = DMReCategorizor(from_file=True,path=opt.build_folder+"dicts/dm_recategorization",training=False)
        self.training = True
    ...
    @staticmethod
    def graph_to_mrpGraph(id, G, normalizeMod=False, flavor = 0, framework = "dm", sentence = None):
        ...
        for e in G.edges:
            ...
            d = e[1]
            role = G.edges[e]["role"]
            if DMGraph.is_inversed_edge(role):
                continue
            elif role == ":top":
    ...
    def probToId(self, srcBatch, src_charBatch, probBatch,sourceBatch):
        ...
        for i,prob in enumerate(probBatch):
            ...
            if i== DM_CAT:
                ...
                # we don't have source to copy for DM
    ...
    def probAndSourceToConcepts(self, sourceBatch,srcBatch, src_charBatch, probBatch,getsense=False):
        ...
        def id_to_high(max_id, idx):
            ...
            if idx == DM_CAT:
                for id in max_id:
                    if id < self.n_high:
                        out.append(self.dicts["dm_high_dict"].getLabel(id))
                    else:
            ...
            #            out.append(self.dicts["dm_high_le_dict"].getLabel(id))
            #        else:
            #            # this part is not differentiable, we leave it for determistic mapping
            #            out.append(None)
        ...
        for i,source in enumerate(sourceBatch):
            ...
            pos = self.dicts["dm_target_pos_dict"].convertToLabels(max_id[:,DM_POS].tolist())
            cats = id_to_high(max_id[:,DM_CAT].tolist(), DM_CAT)
            # pred_lemma = id_to_high(max_id[:,DM_LE].tolist(), DM_LE)
            # here always no cat the sense, we don't use the sense for semi,56 sense
            if DM_SENSE< max_id.size(1):
                sense = self.dicts["dm_sense_dict"].convertToLabels(max_id[:,DM_SENSE].tolist())
            else:
                sense = None

            dm_seq = self.rl.toDMSeq(pos, snt,lemma, None, cats,sense,ners,mwes)
            # TODO: show concepts, align

            # TODO: here adding gold concepts here.
            srl_concept,align,dependent_mark = self.fragment_to_node_converter.unpack_recategorized(dm_seq,self.rl ,getsense,eval= not self.training)
            if len(srl_concept) == 0:
                # TODO: here force it not empty, but nodes may be empty sometimes
                srl_concept = [ DMUniversal.NULL_DMUniversal()]
                align = [0]
                dependent_mark = [0]
                aux_triples = []
        ...
        for n,d in graph.nodes(True):
            ...
            for nearb in graph[n]:
                for key, edge_data in graph[n][nearb].items():
                    if edge_data["role"] in self.dicts["dm_rel_dict"] :
                        r = edge_data["role"]
    ...
    def relProbAndConToGraph(self,srl_batch, sourceBatch, srl_prob,roots,appended,get_sense=False,set_wiki=False,normalizeMod=False):
        ...
        rel_dict = self.dicts["dm_rel_dict"]
        def get_uni_var(concepts,id):
            """
            for dm, id is also not important, just use the id as variable value
            """
            return concepts[id],DMVar(str(id))
        ...
        def create_connected_graph(role_scores,concepts,root_id,dependent,aligns,source):
            ...
            for h_id in range(n):
                for d_id in range(n):
                    if h_id != d_id:
                        ...
                        r_inver = DMGraph.get_inversed_edge(r)
                        h,h_v = get_uni_var(concepts,h_id)
            ...
            for n,d in graph.nodes(True):
                ...
                d["value"] = DMUniversal(pos,cat,sense,le,anchors)
                d["anchors"] = anchors
