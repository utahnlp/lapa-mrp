...
Also the whole DM model is combined here.
...
#Encoding linearized DM concepts for vartiaonal alignment model
class DMTransformerEncoder(nn.Module):

    def __init__(self, opt, embs, transformer_encoder):
        super(DMTransformerEncoder, self).__init__()
        # transformer configuration:
        # opt.dm_transformer_config = "hidden_dim:projection_dim:feedforward_hidden_dim:num_layers:num_atttention_heads:use_positional_encoding:dropout_prob:residual_dropout_prob:attention_dropout_prob"
        self.transformer_encoder = transformer_encoder
        # hidden_dim for stacked_self_att
        inputSize = embs["dm_cat_lut"].embedding_dim +  embs["lemma_lut"].embedding_dim
    ...
    def forward(self, packed_input, hidden=None):
        ...
        cat_embed = self.dm_cat_lut(input[:,DM_CAT])
        lemma_embed = self.lemma_lut(input[:,DM_LE])
