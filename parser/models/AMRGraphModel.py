...
class AmrTransformerEncoder(nn.Module):

    def __init__(self, opt, embs, transformer_encoder):
        super(AmrTransformerEncoder, self).__init__()
        # transformer configuration:
        # opt.amr_transformer_config = "hidden_dim:projection_dim:feedforward_hidden_dim:num_layers:num_atttention_heads:use_positional_encoding:dropout_prob:residual_dropout_prob:attention_dropout_prob"
        self.transformer_encoder = transformer_encoder
        # hidden_dim for stacked_self_att
        inputSize = embs["amr_cat_lut"].embedding_dim +  embs["lemma_lut"].embedding_dim
