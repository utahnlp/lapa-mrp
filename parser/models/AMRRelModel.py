...
class AMRRootEncoder(nn.Module):

    def __init__(self, opt, embs, root_src_enc_size):
        super(AMRRootEncoder, self).__init__()
        ...
        inputSize = embs["amr_cat_lut"].embedding_dim + embs["lemma_lut"].embedding_dim+root_src_enc_size

        self.amr_cat_lut = embs["amr_cat_lut"]
    ...
    def forward(self, input, index,src_enc):
        ...
        cat_embed = self.amr_cat_lut(input[:,AMR_CAT])
...
#combine amr node embedding and aligned sentence token embedding
class AMRRelEncoder(nn.Module):

    def __init__(self, opt, embs, rel_src_enc_size):
        super(AMRRelEncoder, self).__init__()

        self.layers = opt.amr_enlayers
        self.size = opt.rel_dim
        self.rel_src_enc_size = rel_src_enc_size

        self.inputSize = embs["amr_cat_lut"].embedding_dim + embs["lemma_lut"].embedding_dim+ self.rel_src_enc_size
    ...
    def forward(self, input, index,src_enc):
        ...
        # dep_emb_t :  MyDoublePackedSequence(PackedSequenceLength(packed_real_g_amr_l x real_g_amr_l x dim), length_pairs)
        # length_pairs :(g_amr_l, g_amr_l)
        # here we only support a node aligned to a single token. for UCCA, we need to think about it carefully
