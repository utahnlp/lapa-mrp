...
def dm_data_build_parser():
    parser = argparse.ArgumentParser(description='dm_data_build.py')
...
trainingPickleFile = opt.build_folder + "/training/training.dm_pickle_processed"
...
devPickleFile = opt.build_folder + "/dev/dev.dm_pickle_processed"
...
testPickleFile = opt.build_folder + "/test/test.dm_pickle_processed"
...
def mydm_to_seq(dm, snt_token, lemma_token, pos, tok_anchors, ners, mwes, rl, fragment_to_node_converter):  # high_freq should be a dict()
    """
    make a dm to return a squence of node and rels
    ...
    # root_id is the index in gold_dm_index
    ...
    rl.get_matched_concepts(snt_token,dm,lemma_token, pos, tok_anchors, ners, mwes)
    # do recategorization
    fragment_to_node_converter.convert(dm, rl, snt_token, lemma_token, lemma_str)
    # return all concepts, relations, rootid
    # concepts = [[subnode1, subnode-attr], [subnode2, subnode-attr]]
    # rel = [[node, node1ReCateIndex], [[rel, node2GoldIndex]]]], the order is accroding to the index of gold dm index
    # root_id, int, the orderid in all original gold nodes
    concepts, rel, root_id = dm.node_value(keys=["value", "align"], all=True)

    results=rl.get_matched_concepts(snt_token,dm,lemma_token, pos, tok_anchors, ners, mwes)
    ...
    #  assert n_snt <= n_dm or unaligned_index != [],(n_dm,n_snt,concepts,snt_token,dm
    ...
    for i in range(n_snt):
        if i in unaligned_index:
            ...
            output_concepts.append([NULL_WORD, NULL_WORD, NULL_WORD, NULL_WORD, 0,0, [i]])  # len(dm) >= len(snt)

    for i in range(len(output_concepts), n_snt):
        output_concepts.append([NULL_WORD, NULL_WORD, NULL_WORD, NULL_WORD, 0,0, [-1, n_snt]])  # len(dm) >= len(snt)

    logged = False
    for i in range(len(output_concepts)):
        if output_concepts[i][-1] == []:
            if not logged:
                logger.info("output_concepts:\n{}\n snt_token, lemma_token, pos:\n{}\nconcepts:\n{}\n dm:\n{}".format(output_concepts[i], list(zip(snt_token, lemma_token, pos)), concepts, dm))
                logged = True
    ...
    if root_id == None:
        ...
        logger.warn("None root_id for {}".format(dm.id))
        root_id = 0

    # rel is an array, every element is a array [[[node, headnodeReCateIndex], [[rel, node2GoldIndex]]]]
    for dm_index, role_list in rel:
        dm_concept = uni_to_list(dm_index[0])  # if align else  uni_to_list(DMUniversal(UNK_WORD,output_concepts[dm_index[1]][DM_CAT],NULL_WORD))
        # rel feature, contain the DM_POS, DM_CAT, DM_SENSE, DM_LE, DM_CAN_COPY, not alignments
        # in rel_feature, the dm_index is the recategorized index
        rel_feature.append(dm_concept[:6] + [dm_index[1]])
        #     assert dm_index[1] < len(results), (concepts, rel)
        # in rel_tgt, the index for deps are the index from gold dm index
...
def dm_seq_to_id(target_pos_dict, cat_dict, sense_dict, lemma_dict, dm_seq):
...
def dm_seq_to_dict(target_pos_dict, cat_dict, sense_dict, lemma_dict, dm_seq):  # le,cat,le_sense,ner,align
    for i in dm_seq:
...
def rel_seq_to_dict(target_pos_dict, cat_dict, sense_dict, lemma_dict, rel_dict, rel):  # (dm,index,[[role,dm,index]])
    """
...
def rel_seq_to_id(target_pos_dict, cat_dict, sense_dict, lemma_dict, rel_dict, rel):
    ...
    for l in rel_feature:
        ...
        # dm_node index for dm relation transdution, this index is recategorized index
        index_seq.append(l[-1])
    for role_list in rel_tgt:
        roles_id = []
        for role_index in role_list:
            # here role_index[0] is still roleStr, role_index[1], the index when tranverse the role of gold dm_id
    ...
    if build_dict:
        if word_only:
            ...
            if 'mrp_dm' in data:
                ...
                dm_seq, rel, unaligned_index = mydm_to_seq(dm, snt_token, lemma_token, pos, tok_anchors, ner, mwe, rl, fragment_to_node_converter)
                dm_seq_to_dict(target_pos_dict, cat_dict, sense_dict, lemma_dict, dm_seq)
        ...
        if dm_t or 'mrp_dm' in data:
            ...
            data["dm_seq"] = dm_seq
            # all the dm concepts, after recategorizeion. [[subnode1, subnode-attr], [subnode2, subnode]]

            data["dm_convertedl_seq"] = dm.node_value()
            # return all gold concept and rels,
            # cons: [UniversalDM] all gold concepts
            # roles: [head, depend, relStr]
            data["dm_rel_seq"], data["dm_rel_triples"] = dm.get_gold()
            # the target dm ids, include lemma, cat, sense, aux
            data["dm_id"] = dm_seq_to_id(target_pos_dict, cat_dict, sense_dict, lemma_dict, dm_seq)

            data["dm_rel_id"], data["dm_rel_index"], data["dm_roles_mat"], data["dm_root"] = rel_seq_to_id(target_pos_dict,
                cat_dict,
                sense_dict,
                lemma_dict,
                rel_dict, rel)
            ...
            for i in data["dm_rel_index"]:
                assert i < len(data["dm_id"]), (data["dm_rel_index"], dm_seq, data["dm_id"])
            ...
            data["dm_index"] = [all[-1] for all in dm_seq]
...
def buildData(all_data, outPickleFile, build_dict=False, word_only=False):
    ...
    """
    n = 0
    # now only handle the dm data, to extend for all other formats
    dm_data = []
    ...
    logger.info("{} has been preprocessed, {} are good for training".format(n, len(dm_data)))
...
rl.load(opt.build_folder+"dicts/dm_rule_f")
...
non_rule_set = non_rule_set_f.load()["dm_non_rule_set"]
...
non_le_rule_set = non_le_rule_set_f.load()["dm_non_le_rule_set"]
threshold = opt.threshold
...
if not opt.skip:
    ...
    if opt.merge_common_dicts:
        ...
        word_dict = initial_dict(opt.build_folder+"dicts/dm_word_dict", with_unk=True)
        char_dict = initial_dict(opt.build_folder+"dicts/dm_char_dict", with_unk=True)
        lemma_dict = initial_dict(opt.build_folder+"dicts/dm_lemma_dict", with_unk=True)
        source_pos_dict = initial_dict(opt.build_folder+"dicts/dm_source_pos_dict", with_unk=True)
        ner_dict = initial_dict(opt.build_folder+"dicts/dm_ner_dict", with_unk=True)  # from stanford

    # for target, pos, cat, sense, rel
    target_pos_dict = initial_dict(opt.build_folder+"dicts/dm_target_pos_dict", with_unk=True)

    cat_dict = initial_dict(opt.build_folder+"dicts/dm_cat_dict", with_unk=True)

    dm_high_dict = initial_dict(opt.build_folder+"dicts/dm_high_dict", with_unk=True)
    dm_high_le_dict = initial_dict(opt.build_folder+"dicts/dm_high_le_dict", with_unk=True)

    rel_dict = initial_dict(opt.build_folder+"dicts/dm_rel_dict", with_unk=True)

    sense_dict = initial_dict(opt.build_folder+"dicts/dm_sense_dict", with_unk=True)

    for uni in high_freq:
        # for high_dict part, we must keep the same ids
        cat = uni.cat
        id = cat_dict.add(cat)
        dm_high_dict.add(cat, id)

    for uni in high_le_freq:
        # for high_dict part, we must keep the same ids
        le = uni.le
        id = lemma_dict.add(le)
        dm_high_le_dict.add(le, id)
    ...
    #dm_high_dict = gm_high_dict.pruneByThreshold(threshold)
    ...
    logger.info("sense_dict:\n {}".format(sense_dict))
    logger.info("rel_dict:\n {}".format(rel_dict))
    logger.info("dm_high_dict:\n {}".format(dm_high_dict))
    logger.info("dm_high_le_dict:\n {}".format(dm_high_le_dict))
else:
    ...
    if opt.merge_common_dicts:
        ...
        word_dict = Dict(opt.build_folder+"dicts/dm_word_dict")
        char_dict = Dict(opt.build_folder+"dicts/dm_char_dict")
        lemma_dict = Dict(opt.build_folder+"dicts/dm_lemma_dict")
        source_pos_dict = Dict(opt.build_folder+"dicts/dm_source_pos_dict")
        ner_dict = Dict(opt.build_folder+"dicts/dm_ner_dict")

    target_pos_dict = Dict(opt.build_folder+"dicts/dm_target_pos_dict")
    rel_dict = Dict(opt.build_folder+"dicts/dm_rel_dict")
    cat_dict = Dict(opt.build_folder+"dicts/dm_cat_dict")
    dm_high_dict = Dict(opt.build_folder+"dicts/dm_high_dict")
    dm_high_le_dict = Dict(opt.build_folder+"dicts/dm_high_le_dict")
    sense_dict = Dict(opt.build_folder+"dicts/dm_sense_dict")
...
fragment_to_node_converter = DMReCategorizor(from_file=True, path=opt.build_folder+"dicts/dm_recategorization", training=False)
